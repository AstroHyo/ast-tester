...
/**
 * Parses the JavaScript code string using Espree and returns the AST or an error.
 * @param code - The full source code of a file
 * @returns ParseResult containing either the AST or an error message
 */
 ...
 * @returns ASTMetrics
 */
export function analyzeASTMetrics(ast: any): ASTMetrics {
  let functionCount = 0;
...
/**
 * Combines AST analysis into one report.
 * @param code - The source code to analyze.
 * @returns An object containing ASTMetrics and any parse error.
 */
export async function analyzeAST(code: string) {
  // Parse the code to get AST or error
  const parseResult = parseCode(code);

  // If there is a parse error, return report with error
  ...
  const astMetrics = analyzeASTMetrics(parseResult.ast);
